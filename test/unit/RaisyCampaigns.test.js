...
contract(
  "RaisyCampaigns",
  ([owner, projectowner, daotreasuryadd, user, feeAddress]) => {
    ...
    beforeEach(async () => {
      ...
      this.raisyAddressRegistry = await RaisyAddressRegistry.new({
        from: owner,
      });
      ...
      await this.raisyAddressRegistry.updateRaisyToken(
        this.raisyToken.address,
        {
          from: owner,
        }
      ...
      await this.raisyAddressRegistry.updateFeeAddress(feeAddress, {
        from: owner,
      });
      ...
      await this.raisyTokenRegistry.add(this.raisyToken.address, {
        from: owner,
      });
    });
    ...
    describe("claimInitialFunds()", async () => {
      ...
      it("successfuly claim initial funds", async () => {
        ...
        expect(balanceAfter).to.be.bignumber.equal(ether("7312.5"));
      });
    });
    ...
    describe("claimNextFunds()", async () => {
      ...
      it("successfuly claims next funds", async () => {
        ...
        console.log(
          "Balance after claim next funds : ",
